...
app.post("/satelliteSearch", async (req, res) => {
  try {
    ...
    if (/^\d+$/.test(query)) {
      return res.json({
        results: [
          { name: `NORAD ${query}`, noradId: Number(query), category: "Unknown" }
        ]
      });
    }
...
app.post("/getSatellitePosition", async (req, res) => {
  try {
    ...
    if (!noradId) return res.status(400).json({ error: "noradId is required" });
    if (!N2YO_API_KEY) return res.status(500).json({ error: "N2YO_API_KEY missing" });
    ...
    try {
      ...
      return res.status(502).json({ error: "N2YO returned non-JSON", details: text.slice(0, 300) });
    }

    if (!r.ok) {
      return res.status(r.status).json({ error: "N2YO request failed", details: data });
    }

    const info = data.info || {};
    const pos = data.positions?.[0];
    if (!pos) return res.status(404).json({ error: "No satellite position returned", details: data });
    ...
    res.status(500).json({ error: "Position fetch failed", details: err.message });
  }
});
